...
2. Build Neural Networks in PyTorch
3. Activation Functions
4. Working with Tensors
5. Programming Assignment: Deeper Regression, Smarter Features

::: {.notes}
Welcome to Module 1: PyTorch Fundamentals. We'll start with what deep learning is, then build neural networks in PyTorch, explore activation functions, work with tensors, and finish with a programming assignment.
:::
...
Albiet, we use DL for tasks where the data itself is usually not engineerable in the first place.
...
- **DL:** Vanilla NNs, CNNs, RNNs, Transformers
...
- **ML:** Many models are relatively interpretable
  - e.g., in a decision tree, one can trace a decision back to all the intermediate decisions that led to it
- **DL:** Often behaves like a black box and needs post-hoc explanation (XAI research field)
...
Nowadays, we use **Transfer Learning** to apply pre-trained models on smaller datasets.
